		...
		for use_intermediate_embedding in ${use_intermediate_embeddings[@]}; do
			for use_linear_comb in ${use_linear_combs[@]}; do
				for n_hidden in ${n_hiddens[@]}; do
						python3 main.py --data $dataset --use_intermediate_embedding $use_intermediate_embedding --use_linear_comb $use_linear_comb --n_hidden $n_hidden --model_category $model_category --n_epochs 1
				done
			done
		done
	done
