				...
				for n_hidden in ${n_hiddens[@]}; do
					for num_repeats in ${num_repeats_list[@]}; do
						for model in ${models[@]}; do
							python3 main_v2.py --data $dataset --use_intermediate_embedding $use_intermediate_embedding --use_linear_comb $use_linear_comb --n_hidden $n_hidden --model_category $model_category --num_repeats $num_repeats --model $model
						done
					done	
				done
			done
